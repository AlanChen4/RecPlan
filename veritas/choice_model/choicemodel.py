...
class ChoiceModel():
    ...
    def _get_baseline_model(self, user):
        """
        check if there are any recalibrated baseline models belonging to the user
        """
        if BaselineModel.objects.filter(user=user).exists():
            return BaselineModel.objects.get(user=user)
        return None
